...
export default function ProfessionalCoursePage() {
  ...
  return <div className="course-experience">
    ...
    <header className="course-heading visual-course-heading">
      <img className="course-hero-art" src={course.thumbnail || (pentesting ? "/course-art/pentesting-program-v2.png" : "/course-art/osint-program-v2.png")} alt={course.image_alt || ""} />
    ...
    <div className="course-grid"><div className="stack">
      <section className="panel"><div className="eyebrow">Course overview</div><h2>Build knowledge. Put it into practice.</h2>
        ...
        <dl className="facts">{[["Duration", facts.duration], ["Schedule", facts.schedule], ["Class length", facts.class_length], ["Total classes", facts.total_classes], ["Total hours", facts.total_hours_label || facts.total_hours], ["Batch size", facts.batch_size_label || facts.batch_size]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value || "To be announced"}</dd></div>)}</dl>
